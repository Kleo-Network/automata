...
function sendUpdate(message: string, stepIndex?: number, status?: STEP_STATUS) {
  if (port) {
    const update = {
      timestamp: Date.now(),
      message,
...
async function executeActions(actions: ScriptAction[]) {
  ...
  for (let i = 0; i < actions.length; i++) {
    ...
    try {
      ...
      // After successful execution of each action
      sendUpdate(`Completed: ${action.type}`, i, STEP_STATUS.SUCCESS);
    } catch (error) {
      console.error('Background: Action execution error:', error);
      sendUpdate(`Error executing ${action.type}: ${error.message}`, i, STEP_STATUS.ERROR);
      throw error;
    }
  }
